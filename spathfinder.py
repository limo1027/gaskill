...
def astar_cost(cost_grid, start, end, heuristic='manhattan', allow_diagonal=False):
    ...
    Args:
        cost_grid: 2维成本矩阵，值为0表示不可通行，正值表示移动成本
        start: 起点坐标 (x, y)
        end: 终点坐标 (x, y)
        heuristic: 启发式方法 'manhattan', 'euclidean', 'chebyshev', 'octile'
        allow_diagonal: 是否允许对角线移动
    
    Returns:
        路径列表 [vec2(x, y), ...]，None表示无路径
    """
    start = (int(start[0]), int(start[1]))
    end = (int(end[0]), int(end[1]))
...
def create_cost_grid(grid, default_cost=1.0, wall_cost=-1.0, custom_costs=None):
    ...
    Args:
        grid: 二进制网格（0=可通行, 1=障碍物）
        default_cost: 可通行区域的默认成本
        wall_cost: 障碍物成本（-1表示不可通行）
        custom_costs: 自定义成本字典 {(x, y): cost, ...}
