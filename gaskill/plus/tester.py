...
class TestMath(TestCase):
    ...
    def test_fail_demo(self):
        self.assert_equal(2 * 3, 7)  # 故意失败
    
    @skip("除法功能待实现")
    @test
    def test_division(self):
        self.assert_equal(6 / 2, 3)

class TestString(TestCase):
    @test
    def test_upper(self):
        self.assert_equal("hello".upper(), "HELLO")
    
    @test
    def test_lower(self):
        self.assert_equal("WORLD".lower(), "world")
    
    @test
    def test_assert_raises(self):
        def bad_func():
            raise ValueError("错误")
        self.assert_raises(ValueError, bad_func)
