...
from .smath import sqrt, EPSILON, pi
...
def project_3d_to_2d(point, camera_pos, camera_angle, fov=60, width=800, height=600):
    ...
    Args:
        point: vec3 世界坐标中的点
        camera_pos: vec3 相机位置 (x, y, z)
        camera_angle: vec3 相机欧拉角 (pitch, yaw, roll) 弧度
        fov: 视野角度（度），默认 60
        width: 屏幕宽度（像素），默认 800
        height: 屏幕高度（像素），默认 600
    
    Returns:
        (x, y) 屏幕坐标，如果在视野外返回 None
    """
    from .svector import vec3
    from .smath import cos, sin, tan, pi
    ...
    # 2. 用欧拉角构建旋转矩阵（反向旋转相机，相当于把世界转到相机坐标系）
    ...
    screen_y = (1 - ndc_y) * height / 2  # 注意 Y 轴翻转
